...
<head>
    ...
    <title>Créer un Frais - EduMaster</title>
    <style>
...
<body>
    ...
    <div class="container">
        <div class="page-header">
            <h1 class="page-title">➕ Nouveau Frais</h1>
            <p>Renseignez les informations du frais</p>
        </div>
            ...
            <form method="POST" action="{{ route('fees.store') }}">
                    ...
                    <div class="form-group">
                        ...
                        <select id="frequency" name="frequency" required>
                            @foreach($frequencies as $f)
                                <option value="{{ $f }}" {{ old('frequency')==$f?'selected':'' }}>{{ ucfirst($f) }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="form-group">
                        ...
                        <select id="school_year_id" name="school_year_id" required>
                            @foreach($schoolYears as $y)
                                <option value="{{ $y->id }}" {{ old('school_year_id')==$y->id?'selected':'' }}>{{ $y->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group full-width">
                        ...
                        <input type="text" id="class_level" name="class_level" value="{{ old('class_level') }}" placeholder="Ex: CM2, 4ème ...">
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group full-width">
                        ...
                        <textarea id="description" name="description" rows="3" placeholder="Détails...">{{ old('description') }}</textarea>
                    </div>
                </div>

                <div class="form-actions">
                    <button type="submit" class="btn btn-primary">✅ Enregistrer le Frais</button>
                    <a href="{{ route('fees.index') }}" class="btn btn-secondary">❌ Annuler</a>
                </div>
            </form>
        </div>
    </div>
