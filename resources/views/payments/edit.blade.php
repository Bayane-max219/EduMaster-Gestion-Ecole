...
<head>
    ...
    <title>Modifier un Paiement - EduMaster</title>
    <style>
...
<body>
    ...
    <div class="container">
        <div class="page-header">
            <h1 class="page-title">✏️ Modifier le Paiement</h1>
            ...
            <form method="POST" action="{{ route('payments.update', $payment) }}">
                    ...
                    <div class="form-group">
                        ...
                        <select id="student_id" name="student_id" required>
                            ...
                            @foreach($students as $st)
                                <option value="{{ $st->id }}" {{ old('student_id', $payment->student_id)==$st->id?'selected':'' }}>{{ $st->full_name ?? $st->name }}</option>
                            @endforeach
                        </select>
                        <div id="student-class-display" class="helper">Classe actuelle : —</div>
                    </div>
                    <div class="form-group">
                        ...
                        <select id="fee_id" name="fee_id" required>
                            ...
                            @foreach($fees as $f)
                                <option data-amount="{{ $f->amount }}" value="{{ $f->id }}" {{ old('fee_id', $payment->fee_id)==$f->id?'selected':'' }}>{{ $f->name }} - {{ number_format((float)$f->amount, 0, ',', ' ') }} Ar</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" id="payment_date" name="payment_date" value="{{ old('payment_date', optional($payment->payment_date)->format('Y-m-d')) }}" required>
                    </div>
                </div>
                    ...
                    <div class="form-group">
                        ...
                        <select id="payment_method" name="payment_method" required>
                            @foreach($methods as $m)
                                <option value="{{ $m }}" {{ old('payment_method', $payment->payment_method)==$m?'selected':'' }}>{{ ucwords(str_replace('_',' ', $m)) }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <select id="status" name="status" required>
                            @foreach($statuses as $s)
                                <option value="{{ $s }}" {{ old('status', $payment->status)==$s?'selected':'' }}>{{ $statusLabels[$s] ?? ucfirst($s) }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" id="reference" name="reference" value="{{ old('reference', $payment->reference) }}" placeholder="Ref. chèque / virement ...">
                    </div>
                    <div class="form-group">
                        <label for="receipt_number">N° Reçu</label>
                        <input type="text" id="receipt_number" name="receipt_number" value="{{ old('receipt_number', $payment->receipt_number) }}" placeholder="Ex: R-2025-00123">
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group full-width">
                        ...
                        <textarea id="notes" name="notes" rows="3" placeholder="Commentaires...">{{ old('notes', $payment->notes) }}</textarea>
                    </div>
                </div>

                <div class="form-actions">
                    <button type="submit" class="btn btn-primary">💾 Enregistrer</button>
                    <a href="{{ route('payments.show', $payment) }}" class="btn btn-secondary">↩️ Annuler</a>
                </div>
            </form>

            <div class="danger-zone">
                <div style="font-weight:700;color:#dc3545;margin-bottom:.5rem;">Zone Dangereuse</div>
                <p style="margin-bottom: .75rem; color:#333;">Supprimer définitivement ce paiement.</p>
