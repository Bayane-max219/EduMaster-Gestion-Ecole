...
<head>
    ...
    <title>Rapport des Notes - PDF</title>
    <style>
...
<body>
    ...
    <div class="container">
        <h1 class="title">Rapport des Notes</h1>
        @if(!$has)
            <div class="card">Aucune donnée disponible (table grades manquante).</div>
        @else
            <div class="card" style="display:grid; grid-template-columns:1fr 1fr; gap:1rem;">
                <div>
                    <div style="color:#555;">Total enregistrements</div>
                    <div class="stat">{{ number_format((int)$total, 0, ',', ' ') }}</div>
                </div>
                <div>
                    <div style="color:#555;">Moyenne générale</div>
                    <div class="stat">{{ number_format((float)$avg, 2, ',', ' ') }}</div>
                </div>
            </div>

            <div class="card">
                <h3 style="color:#20B2AA; margin-bottom:.5rem;">Répartition par matière</h3>
                <table>
                    <thead><tr><th>Matière</th><th>Total</th></tr></thead>
                    <tbody>
                    @forelse($bySubject as $row)
                        <tr>
                            <td>{{ ($subjects[$row->subject_id] ?? ('Matière #'.$row->subject_id)) }}</td>
                            <td>{{ $row->count }}</td>
                        </tr>
                    @empty
