...
<head>
    ...
    <title>Modifier une Note - EduMaster</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .danger-zone { background: #fff; border: 2px solid #ffe1e1; padding: 1rem; border-radius: 12px; margin-top: 2rem; }
    </style>
</head>
<body>
    <div class="header">
    ...
    <div class="container">
        <div class="page-header">
            ...
            <p>Mettez à jour les informations de l'évaluation</p>
        </div>
            ...
            <form method="POST" action="{{ route('grades.update', $grade) }}">
                    ...
                    <div class="form-group">
                        ...
                        <select id="student_id" name="student_id" required>
                            ...
                            @foreach($students as $st)
                                <option value="{{ $st->id }}" {{ old('student_id', $grade->student_id)==$st->id?'selected':'' }}>{{ $st->full_name ?? $st->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <select id="class_id" name="class_id" required>
                            ...
                            @foreach($classes as $c)
                                <option value="{{ $c->id }}" {{ old('class_id', $grade->class_id)==$c->id?'selected':'' }}>{{ $c->name }} ({{ $c->level }})</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="form-group">
                        ...
                        <select id="teacher_id" name="teacher_id" required>
                            ...
                            @foreach($teachers as $t)
                                <option value="{{ $t->id }}" {{ old('teacher_id', $grade->teacher_id)==$t->id?'selected':'' }}>{{ $t->full_name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="form-group">
                        ...
                        <select id="exam_type" name="exam_type" required>
                            ...
                            @foreach(['devoir','controle','examen','oral','pratique'] as $type)
                                <option value="{{ $type }}" {{ old('exam_type', $grade->exam_type)==$type?'selected':'' }}>{{ ucfirst($type) }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="exam_name">Intitulé <span class="required">*</span></label>
                        <input type="text" id="exam_name" name="exam_name" value="{{ old('exam_name', $grade->exam_name) }}" placeholder="Ex: Contrôle chapitre 2" required>
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="number" id="score" name="score" value="{{ old('score', $grade->score) }}" step="0.01" min="0">
                    </div>
                    <div class="form-group">
                        <label for="max_score">Note maximale <span class="required">*</span></label>
                        <input type="number" id="max_score" name="max_score" value="{{ old('max_score', $grade->max_score) }}" step="0.01" min="1" max="100" required>
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="date" id="exam_date" name="exam_date" value="{{ old('exam_date', optional($grade->exam_date)->format('Y-m-d')) }}" required>
                    </div>
                    <div class="form-group">
                        <label for="semester">Semestre <span class="required">*</span></label>
                        <select id="semester" name="semester" required>
                            <option value="1" {{ old('semester', $grade->semester)=='1'?'selected':'' }}>1</option>
                            <option value="2" {{ old('semester', $grade->semester)=='2'?'selected':'' }}>2</option>
                        </select>
                    </div>
                </div>
                    ...
                    <div class="form-group full-width">
                        ...
                        <select id="school_year_id" name="school_year_id" required>
                            @foreach($schoolYears as $y)
                                <option value="{{ $y->id }}" {{ old('school_year_id', $grade->school_year_id)==$y->id?'selected':'' }}>{{ $y->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group full-width">
                        ...
                        <textarea id="notes" name="notes" rows="3" placeholder="Commentaires...">{{ old('notes', $grade->notes) }}</textarea>
                    </div>
                </div>

                <div class="form-actions">
                    <button type="submit" class="btn btn-primary">💾 Enregistrer</button>
                    <a href="{{ route('grades.show', $grade) }}" class="btn btn-secondary">↩️ Annuler</a>
                </div>
            </form>

            <div class="danger-zone">
                <div style="font-weight:700;color:#dc3545;margin-bottom:.5rem;">Zone Dangereuse</div>
                <p style="margin-bottom: .75rem; color:#333;">Supprimer définitivement cette note.</p>
    ...
    @php
        // Mapping élève -> classe (via nom de classe)
        $studentClassMap = [];
        foreach ($students as $st) {
            $classId = null;
            if (!empty($st->classe)) {
        ...
        // Mapping professeur <-> matières
        $teacherSubjects = [];
        $subjectTeachers = [];

        if (\Illuminate\Support\Facades\Schema::hasTable('teacher_subjects')
            && \Illuminate\Support\Facades\Schema::hasTable('subjects')) {
